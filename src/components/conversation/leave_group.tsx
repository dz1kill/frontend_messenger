  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      if (data.success && data.type === TYPE_LEAVE_GROUP) {
        ...
        if (data.params.isBroadcast || !currentConversation?.groupId) return;

        dispatch(removeLastMessageByGroupId(currentConversation.groupId));
        dispatch(removeGroupMessages(currentConversation.groupId));
